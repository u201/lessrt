 ...
 * \ingroup libcore
 */
template <typename T> struct TSpectrum {
public:
	typedef T          Scalar;
	...
	explicit inline TSpectrum(Scalar* spec) {
		memcpy(s.data(), spec, sizeof(Scalar)*SPECTRUM_SAMPLES);
	}

	/// Unserialize a spectral power distribution from a binary data stream
	explicit inline TSpectrum(Stream *stream) {
		stream->readArray(s.data(), SPECTRUM_SAMPLES);
	}

	/// Initialize with a TSpectrum data type based on a alternate representation
	template <typename AltScalar> explicit TSpectrum(const TSpectrum<AltScalar> &v) {
		for (int i = 0; i<SPECTRUM_SAMPLES; ++i)
			s[i] = (Scalar)v[i];
	}
	...
	inline void serialize(Stream *stream) const {
		stream->writeArray(s.data(), SPECTRUM_SAMPLES);
	}
...
protected:
	//Scalar s[SPECTRUM_SAMPLES];
	std::vector<Scalar> s = std::vector<Scalar>(SPECTRUM_SAMPLES);
};
...
// * \ingroup libpython
// */
//struct MTS_EXPORT_CORE Color3 : public TSpectrum<Float> {
//public:
//	typedef TSpectrum<Float> Parent;
//
 ...
 * \ingroup libpython
 */
struct MTS_EXPORT_CORE Spectrum : public TSpectrum<Float> {
public:
	typedef TSpectrum<Float> Parent;
	...
	/// Initialize with a TSpectrum data type based on a alternate representation
	template <typename AltScalar> explicit Spectrum(const TSpectrum<AltScalar> &v) {
		for (int i=0; i<SPECTRUM_SAMPLES; ++i)
			s[i] = (Scalar) v[i];
	}
	...
	/// Copy a spectral power distribution
	explicit inline Spectrum(Float *value) {
		memcpy(s.data(), value, sizeof(Float)*SPECTRUM_SAMPLES);
	}
...
protected:
	...
	static std::vector<Float> m_wavelengths;
...
/**
* \brief Abstract spectral power distribution data type for Color
* which is mainly for representing color3 and color1
*
*
* \ingroup libcore
*/
template <typename T, int N> struct TSpectrumColor {
public:
	typedef T          Scalar;
	...
	/// Create a new spectral power distribution with all samples set to the given value
	explicit inline TSpectrumColor(Scalar v) {
		for (int i = 0; i<N; i++)
			s[i] = v;
	}

	/// Copy a spectral power distribution
	explicit inline TSpectrumColor(Scalar spec[N]) {
		memcpy(s, spec, sizeof(Scalar)*N);
	}

	/// Unserialize a spectral power distribution from a binary data stream
	explicit inline TSpectrumColor(Stream *stream) {
		stream->readArray(s, N);
	}

	/// Initialize with a TSpectrumColor data type based on a alternate representation
	template <typename AltScalar> explicit TSpectrumColor(const TSpectrumColor<AltScalar, N> &v) {
		for (int i = 0; i<N; ++i)
			s[i] = (Scalar)v[i];
	}

	/// Add two spectral power distributions
	inline TSpectrumColor operator+(const TSpectrumColor &spec) const {
		TSpectrumColor value = *this;
		for (int i = 0; i<N; i++)
			value.s[i] += spec.s[i];
		return value;
	}
	...
	/// Add a spectral power distribution to this instance
	inline TSpectrumColor& operator+=(const TSpectrumColor &spec) {
		for (int i = 0; i<N; i++)
			s[i] += spec.s[i];
		return *this;
	}
	...
	/// Multiply by a scalar
	inline TSpectrumColor operator*(Scalar f) const {
		TSpectrumColor value = *this;
		for (int i = 0; i<N; i++)
			value.s[i] *= f;
		return value;
	...
	/// Multiply by a scalar
	inline friend TSpectrumColor operator*(Scalar f, const TSpectrumColor &spec) {
		return spec * f;
	}

	/// Multiply by a scalar
	inline TSpectrumColor& operator*=(Scalar f) {
		for (int i = 0; i<N; i++)
			s[i] *= f;
		return *this;
	}
...
#ifdef MTS_DEBUG
		if (f == 0)
			SLog(EWarn, "TSpectrumColor: Division by zero!");
#endif
		Scalar recip = 1.0f / f;
		for (int i = 0; i<N; i++)
			value.s[i] *= recip;
	...
	/// Equality test
	inline bool operator==(const TSpectrumColor &spec) const {
		for (int i = 0; i<N; i++) {
			if (s[i] != spec.s[i])
				return false;
		}
	...
	/// Divide by a scalar
	inline friend TSpectrumColor operator/(Scalar f, TSpectrumColor &spec) {
		return TSpectrumColor(f) / spec;
	}
	...
	/// Multiply-accumulate operation, adds \a weight * \a spec
	inline void addWeighted(Scalar weight, const TSpectrumColor &spec) {
		for (int i = 0; i<N; i++)
			s[i] += weight * spec.s[i];
	}
	...
	/// Component-wise absolute value
	inline TSpectrumColor abs() const {
		TSpectrumColor value;
		for (int i = 0; i<N; i++)
			value.s[i] = std::abs(s[i]);
		return value;
	}
	...
	/// Component-wise square root
	inline TSpectrumColor sqrt() const {
		TSpectrumColor value;
		for (int i = 0; i<N; i++)
			value.s[i] = std::sqrt(s[i]);
		return value;
	...
	/// Component-wise square root
	inline TSpectrumColor safe_sqrt() const {
		TSpectrumColor value;
		for (int i = 0; i<N; i++)
			value.s[i] = math::safe_sqrt(s[i]);
		return value;
	...
	/// Component-wise logarithm
	inline TSpectrumColor log() const {
		TSpectrumColor value;
		for (int i = 0; i<N; i++)
			value.s[i] = math::fastlog(s[i]);
		return value;
	...
	/// Component-wise exponentation
	inline TSpectrumColor exp() const {
		TSpectrumColor value;
		for (int i = 0; i<N; i++)
			value.s[i] = math::fastexp(s[i]);
		return value;
	...
	/// Component-wise power
	inline TSpectrumColor pow(Scalar f) const {
		TSpectrumColor value;
		for (int i = 0; i<N; i++)
			value.s[i] = std::pow(s[i], f);
		return value;
	...
	/// Negate
	inline TSpectrumColor operator-() const {
		TSpectrumColor value;
		for (int i = 0; i<N; i++)
			value.s[i] = -s[i];
		return value;
...
* \ingroup libpython
*/
struct MTS_EXPORT_CORE Color3 : public TSpectrumColor<Float, 3> {
public:
	typedef TSpectrumColor<Float, 3> Parent;
